    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-2 text-sm">
                ...
                @if($gig->price_summary['total'] === '0.00')
                    <p class="text-yellow-600 italic text-xs">No prices set yet. Edit to add pricing for subtopics.</p>
                @endif
            </div>
        </div>
    </div>
                                        ...
                                        @foreach($topic->subtopics as $subtopic)
                                            <div class="flex justify-between text-sm">
                                                ...
                                                <div class="flex items-center gap-4 text-xs text-gray-500">
                                                    <span>Duration: {{ $subtopic->duration }} min ({{ number_format($subtopic->duration / 60, 1) }}H)</span>
                                                    {{-- NEW: Display Subtopic Price --}}
                                                    <span>Price: LKR {{ number_format($subtopic->price ?? 0, 2) }}</span>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
