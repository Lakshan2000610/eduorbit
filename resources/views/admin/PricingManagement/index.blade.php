...
<script>
  ...
  function renderGrades() {
    ...
    if (currentData.grades.length === 0) {
      html += `<tr><td colspan="5" class="px-6 py-4 text-center text-gray-500">No grades available.</td></tr>`;
    } else {
  ...
  function renderSubjects(grade) {
    ...
    if (grade.subjects.length === 0) {
      html += `<tr><td colspan="5" class="px-6 py-4 text-center text-gray-500">No subjects available.</td></tr>`;
    } else {
  ...
  function renderTopics(subject) {
    ...
    if (subject.topics.length === 0) {
      html += `<tr><td colspan="5" class="px-6 py-4 text-center text-gray-500">No topics available.</td></tr>`;
    } else {
  ...
  function renderSubtopics(topic) {
    ...
    if (topic.subtopics.length === 0) {
      html += `<tr><td colspan="6" class="px-6 py-4 text-center text-gray-500">No subtopics available.</td></tr>`;
    } else {
  ...
  // Editing subtopics (now with AJAX and CSRF safety)
  ...
  function saveEdit(code) {
    ...
    if (!csrfToken) {
      alert('CSRF token missing. Please refresh the page and ensure your layout includes the CSRF meta tag.');
      return;
    }
    fetch("{{ route('admin.subtopic-pricing.update') }}", {
      method: 'POST',
      headers: {
  ...
  function savePlatformFee() {
    ...
    const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
    if (!csrfToken) {
      document.getElementById("platform-msg").textContent = 'CSRF token missing. Please refresh the page and ensure your layout includes the CSRF meta tag.';
      document.getElementById("platform-msg").className = "text-red-600";
      return;
    }
